...
const MessagesPage = () => {
    ...
    useEffect(() => {
        if (socket) {
            const handleNewMessage = (message: Message & { senderId: string }) => {
                // Only update if the message is from the currently selected conversation
                if (message.senderId === selectedConversation?.id) {
                    setMessages((prevMessages) => [...prevMessages, message]);
                }
                // You could also add a notification for messages from other conversations
            };
    ...
    const handleSendMessage = (content: string) => {
        ...
        const messageData = {
            _id: Date.now().toString(), // Create a temporary ID for optimistic update
            senderId: user.id,
...
export default MessagesPage;import { Layout } from '@/components/layout/Layout';
import { ConversationList, Conversation } from '@/components/chat/ConversationList';
import { ChatWindow, Message } from '@/components/chat/ChatWindow';
import { useSocket } from '@/context/SocketContext';
import { useAuth } from '@/context/AuthContext';

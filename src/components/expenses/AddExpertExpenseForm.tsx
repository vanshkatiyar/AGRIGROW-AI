// This file is specific to Experts
import { useForm } from "react-hook-form";
// ... (rest of the imports are the same as the buyer form)
...
// --- EXPERT-SPECIFIC CATEGORIES ---
const expenseCategories = ["Software", "Travel", "Research", "Marketing", "Other"];
const incomeCategories = ["Consultation Fee", "Article Payout", "Other"];

// The props interface remains the same
interface AddExpertExpenseFormProps {
  onSubmit: (data: z.infer<typeof formSchema>) => void;
  onClose: () => void;
  isPending: boolean;
}

export const AddExpertExpenseForm: React.FC<AddExpertExpenseFormProps> = ({ onSubmit, onClose, isPending }) => {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        {/* Form structure is identical, just categories and placeholders change */}
        <FormField control={form.control} name="type" render={({ field }) => (
          <FormItem><FormControl><RadioGroup onValueChange={field.onChange} defaultValue={field.value} className="flex space-x-4"><FormItem className="flex items-center space-x-2 space-y-0"><FormControl><RadioGroupItem value="expense" /></FormControl><FormLabel className="font-normal flex items-center gap-2"><TrendingDown className="h-4 w-4 text-red-500" /> Expense</FormLabel></FormItem><FormItem className="flex items-center space-x-2 space-y-0"><FormControl><RadioGroupItem value="income" /></FormControl><FormLabel className="font-normal flex items-center gap-2"><TrendingUp className="h-4 w-4 text-green-500" /> Earning</FormLabel></FormItem></RadioGroup></FormControl><FormMessage /></FormItem>
        )} />
        <FormField control={form.control} name="title" render={({ field }) => (<FormItem><FormLabel>Title</FormLabel><FormControl><Input placeholder="e.g., Consultation for Ravi Kumar" {...field} /></FormControl><FormMessage /></FormItem>)} />
